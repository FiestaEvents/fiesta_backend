...
const calculateFinancials = (services, vatRate = 19, stampDuty = 1.000) => {
...
export const getContracts = asyncHandler(async (req, res) => {
  const {
    page = 1, limit = 10, status, contractType, search,
    sortBy = "createdAt", sortOrder = "desc", includeArchived = false,
  } = req.query;
...
export const updateContract = asyncHandler(async (req, res) => {
  ...
  if (req.body.services || req.body.financials) {
    ...
    const vatRate = req.body.financials?.vatRate !== undefined 
      ? req.body.financials.vatRate 
      : contract.financials.vatRate;
      
    const stampDuty = req.body.financials?.stampDuty !== undefined 
      ? req.body.financials.stampDuty 
      : contract.financials.stampDuty;

    const calculatedStats = calculateFinancials(processedServices, vatRate, stampDuty);
  ...
  const allowedUpdates = ["title", "party", "logistics", "paymentTerms", "legal", "status", "event"];
...
export const deleteContract = asyncHandler(async (req, res) => {
  ...
  if (!contract) throw new ApiError(404, "Contract not found");
  if (contract.status === "signed") throw new ApiError(400, "Cannot delete signed contract");
...
export const sendContract = asyncHandler(async (req, res) => {
  const contract = await Contract.findOne({ _id: req.params.id, venue: req.user.venueId });
  if (!contract) throw new ApiError(404, "Contract not found");
...
export const duplicateContract = asyncHandler(async (req, res) => {
  ...
  res.status(201).json(new ApiResponse(201, { contract: newContract }, "Contract duplicated"));
});
...
export const getContractStats = asyncHandler(async (req, res) => {
  const stats = await Contract.aggregate([
    { $match: { venue: req.user.venueId } },
    {
      $group: {
        ...
        // If it's Signed AND Client => Revenue
        revenue: {
          $sum: { 
            $cond: [
              { $and: [{ $eq: ["$status", "signed"] }, { $eq: ["$contractType", "client"] }] }, 
              "$financials.totalTTC", 0
            ] 
          },
        },
        // If it's Signed AND Partner => Expense
        expenses: {
          $sum: { 
            $cond: [
              { $and: [{ $eq: ["$status", "signed"] }, { $eq: ["$contractType", "partner"] }] }, 
              "$financials.totalTTC", 0
            ] 
          },
        },
  ...
  res.status(200).json(
    new ApiResponse(200, stats[0] || { 
      total: 0, draft: 0, signed: 0, pendingSignatures: 0, signedValue: 0, totalValue: 0 
    })
  );
});
...
export const downloadContractPdf = asyncHandler(async (req, res) => {
  ...
  // ...
// Helper: Generate ID based on Settings
const generateContractNumber = async (venueId) => {
  // 1. Fetch Settings
  ...
  // If resetSequenceYearly is true, we count only this year's docs
  ...
  const sequence = String(nextNum).padStart(structure.sequenceDigits, "0");

  // 3. Build String
  let contractNumber = structure.prefix;
