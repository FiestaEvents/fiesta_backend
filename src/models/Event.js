...
const eventSchema = new mongoose.Schema(
  {
    ...
    // - For Photographers: Null (unless booking specific gear)
...
eventSchema.pre("save", async function (next) {
  ...
  if (this.resourceId) {
    ...
    // Check if THIS specific resource is booked elsewhere
    conflictQuery.resourceId = this.resourceId;
  } else {
    // B. HUMAN/GLOBAL BASED (Solo Photographer / Freelancer)
    // If no resource is specified, it implies the *person* (the business owner) is the resource.
    // We check if *any* event exists for this business in this slot that also has no resource specific assignment,
    // OR we can decide that a solo biz can only do one thing at a time period.
    
    // For now, we only block other "global" events (where resourceId is null)
    // allowing complex businesses to have 1 Event in a Room and 1 Event in another Room.
